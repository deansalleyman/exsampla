...
const publishResearchEpic = ( action$ , state$ ) => action$.pipe(
  tap(item => console.log('tap passed action', item, state$.value)),
  filter(action => action.type === researchConstants.SUBMIT),
  // filter(() => (state$.value.research && state$.value.research.sessions && state$.value.research.sessions.length)),
  switchMap(item => from(state$.value.research.sessions)),
  tap(item => console.log('tap passed publishResearchEpic session', item)),
  switchMap(session =>{
