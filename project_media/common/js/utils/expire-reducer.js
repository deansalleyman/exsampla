...
const transformRehydrate = (outboundState, config) => {
  ...
  if (
    (config.expireSeconds || config.expireDays) &&
    outboundState[config.persistedAtKey]
  ) {
    ...
    if (config.expireSecond) {
      ...
      const days = Math.round(duration / oneDay)

      // If the state is older than the set expiry time,
      // reset it to initial state
      if (days > config.expireDays) {
        return Object.assign({}, config.expiredState);
      }
    }
  }
...
function expireReducer(reducerKey, config = {}) {
  const defaults = {
    ...
    expireSeconds: null,
    // Days after which store will be expired
    expireDays: null,
